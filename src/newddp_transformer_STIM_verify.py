...
import math
import numpy as np
from window_dataset_STIM import time_series_dataset2, collate_func2, MAX_SEQ_LEN, FEATURE_DIM
...
class BinaryTransformer5(nn.Module):
    def __init__(self, input_dim=FEATURE_DIM, d_model=128, nhead=8,
        ...
        # sn embedding
        self.wind_feature_embedding = nn.Sequential(
            nn.Linear(66, 64),
            nn.LayerNorm(64),
            nn.GELU(),
            nn.Dropout(0.2),
    ...
    def forward(self, x, mask, win_feature):
        # x: (B, L, F), mask: (B, L)
        seq_len = x.size(1)
        ...
        # Window 特征嵌入
        win_embedding = self.wind_feature_embedding(win_feature)

        # 特征融合
        combine = torch.cat([x[:,-1,:], win_embedding], dim=1)

        # 分类输出
        return self.classifier(combine).squeeze(-1)
...
def get_parse_args():
    ...
    parser.add_argument('--model_type', type=int, default=1,
                        help='1, 2')
    parser.add_argument('--ModelWithLoss_Type', type=int, default=1,
                        help='1, 2')
...
class Trainer(object):
    def __init__(self, args, model, optimizer):
        ...
        print(args.device)
        if args.ModelWithLoss_Type == 1:
            self.model_with_loss = ModelWithLoss(model, args.gpus, args.device)
        elif  args.ModelWithLoss_Type == 2:
            self.model_with_loss = ModelWithLoss2(model, args.gpus, args.device)
...
class ModelWithLoss(torch.nn.Module):
    def __init__(self, model, gpus, device):
        ...
        self.gpus = gpus
        self.device = device
        # self.criterion = focal_loss(reduction="mean", gamma = 2, alpha = 0.50)
...
class ModelWithLoss2(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        labels = batch['label'].float().to(self.device)
        masks = batch['mask'].to(self.device)
        window_features = batch['window_features'].to(self.device)
        
        outputs = self.model(inputs, masks, window_features)
        loss = self.criterion(outputs, labels)
        loss_stats = {'loss': loss}
        return outputs, labels, loss_stats
...
def main(args):
    ...
    # Dataset
    #################
    training_dataset_path = f"/mnt/zhangrengang/data/STIM_Data_train_01-04/STIM_win_feature_obv30d_neg60d_{args.sn_type}/"
    val_dataset_path = f"/mnt/zhangrengang/data/STIM_Data_val_04-06/STIM_win_feature_obv30d_neg30d_{args.sn_type}/"
    if args.local_rank == 0:
        print('\033[32m====> Using settings {}\033[0m'.format(args))
        print('==> Loading training dataset from: ', training_dataset_path)
        print('==> Loading val dataset from: ', val_dataset_path)
    train_ds = time_series_dataset2(training_dataset_path, is_train=True, is_aug_pos = False)
    val_ds = time_series_dataset2(val_dataset_path, is_train=True, is_aug_pos = False)
    ...
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, num_workers=args.num_workers,
                              shuffle=False, collate_fn=collate_func2, sampler=train_sampler)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, num_workers=args.num_workers,
                            shuffle=False, collate_fn=collate_func2, sampler=val_sampler)
