    ...
    def _get_processed_df(self, sn_file: str) -> pd.DataFrame:
        ...
        if os.path.exists(os.path.join(processed_df_files_dir, sn_file.split('.')[0] + '.feather')):
            processed_df = feather.read_dataframe(os.path.join(
                processed_df_files_dir, sn_file.split('.')[0] + '.feather'))
            return processed_df
        ...
        feather.write_dataframe(
            processed_df,
            os.path.join(os.path.join(processed_df_files_dir,
        ...
        if self.ticket_sn_map.get(sn_name):
            ...
            for i in range(len(windows_dict["end_times"])):
                if (windows_dict["end_times"][i] <= end_time) & (windows_dict["end_times"][i] >= start_time):
                    pos_windows_dict["start_indices"].append(
                        windows_dict["start_indices"][i])
                    pos_windows_dict["end_indices"].append(
                        windows_dict["end_indices"][i])
                    pos_windows_dict["end_times"].append(
                        windows_dict["end_times"][i])
...
class NegativeDataGenerator(DataGenerator):
    def _process_file(self, sn_file: str):
        ...
        if not self.ticket_sn_map.get(sn_name):
            ...
            for i in range(len(windows_dict["end_times"])):
                if (windows_dict["end_times"][i] <= end_time) & (windows_dict["end_times"][i] >= start_time):
                    neg_windows_dict["start_indices"].append(
                        windows_dict["start_indices"][i])
                    neg_windows_dict["end_indices"].append(
                        windows_dict["end_indices"][i])
                    neg_windows_dict["end_times"].append(
                        windows_dict["end_times"][i])
    ...
    def _process_file(self, sn_file: str) -> Union[pd.DataFrame, None]:
        ...
        for i in range(len(windows_dict["end_times"])):
            if (windows_dict["end_times"][i] <= self.test_end_date) & (windows_dict["end_times"][i] >= self.test_start_date):
                test_windows_dict["start_indices"].append(
                    windows_dict["start_indices"][i])
                test_windows_dict["end_indices"].append(
                    windows_dict["end_indices"][i])
                test_windows_dict["end_times"].append(
                    windows_dict["end_times"][i])
...
class MFPmodel(object):
    ...
    def predict_proba(self) -> Dict[str, List]:
        ...
        for file in os.listdir(self.test_data_path):
            test_df = feather.read_dataframe(
                os.path.join(self.test_data_path, file))
    ...
    def predict(self, threshold: int = 0.5) -> Dict[str, List]:
        ...
        # 将预测结果按照阈值进行筛选
        result = {
            sn: [int(sn_t.timestamp())
                 for sn_t, p_s in pred_list if p_s >= threshold]
...
def process_windows(windows_dir: str, processed_df_dir: str, output_dir: str, chunk_size: int = 32):
    ...
    for filename in tqdm(file_list, desc=f"processing {windows_dir.split('/')[-1].split('_')[0]} windows"):
        ...
        with open(os.path.join(windows_dir, filename)) as win_file:
            windows_dict = json.load(win_file)
            win_file.close()
        processed_df = feather.read_dataframe(
            os.path.join(processed_df_dir, sn_name + '.feather'))
        for i in range(len(windows_dict["start_indices"])):
            # ----------------------------- data frame process begin ----------------------------------
            win_df = processed_df.iloc[windows_dict["start_indices"]
                                       [i]:windows_dict["end_indices"][i]]
            # -----------------统计window长度
            lens = win_df.shape[0]
            ...
            # -----------------window feature generate
            TIME_RELATED_LIST = [15 * ONE_MINUTE, ONE_HOUR, 6 * ONE_HOUR]
            log_times = win_df["LogTime"].values
            end_logtime_of_filtered_window_df = win_df["LogTime"].values.max()
            window_feature = []
            for time_window_size in TIME_RELATED_LIST:
                ...
                win_df_copy = win_df.iloc[index:]
                # 提取时间特征、空间特征和奇偶校验特征
                temporal_features = zrg_get_temporal_features(win_df_copy, time_window_size).values()
                temporal_features_list = [float(value) for value in temporal_features]
                
                spatio_features = zrg_get_spatio_features(win_df_copy).values()
                spatio_features_list = [float(value) for value in spatio_features]
                
                err_parity_features = zrg_get_err_parity_features(win_df_copy).values()
                err_parity_features_list = [float(value) for value in err_parity_features]
                
                window_feature = window_feature + temporal_features_list + spatio_features_list + err_parity_features_list
            
            # -----------------remove unused features
            win_df = win_df.drop('CellId', axis=1).drop('position_and_parity', axis=1)
            # ----------------------------- data frame process endin ----------------------------------
            ...
            buffer_dict["window_features"].append(window_feature)
...
if __name__ == "__main__":
    ...
    if test_stage == 1:
        test_data_range: tuple = (
            # 第一阶段测试数据范围
            f"2024-{args.s1_train_end_month}-01", f"2024-{args.s1_test_end_month}-01")
    else:
