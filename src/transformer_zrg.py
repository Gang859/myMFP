...
import math
import numpy as np
from window_dataset import time_series_dataset, collate_func
...
if __name__ == '__main__':
    ...
    # 假设已经准备好数据集
    train_loader = DataLoader(train_ds, batch_size=32, num_workers=64,
                             shuffle=True, collate_fn=collate_func)
    val_loader = DataLoader(test_ds, batch_size=4, num_workers=64,
                            shuffle=True,collate_fn=collate_func)
    
    # 训练循环
    best_score = 0
    for epoch in range(1, 41):
        train_loss, acc, f1, auc = train_epoch(model, train_loader, criterion, optimizer, device)
        ...
        if val_metrics['auc'] > best_score:
            ...
            torch.save(model.state_dict(), '/mnt/zhangrengang/model/best_model.pth')
