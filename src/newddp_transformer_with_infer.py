...
class Trainer(object):
    ...
    def run_epoch(self, phase, epoch, dataset, local_rank):
        ...
        for iter_id, batch in enumerate(dataset):
            ...
            # --------------------------- loss calculate
            outputs, labels, loss_stats, _, _ = model_with_loss(batch)
    ...
    def run_infer(self, dataset, local_rank, sn_type, output_dir):
        ...
        for iter_id, batch in enumerate(dataset):
            ...
            data_time.update(time.time() - end)
            # --------------------------- loss calculate
            outputs, _, _, sn_names, log_times = model_with_loss(
...
class ModelWithLoss(torch.nn.Module):
    ...
    def forward(self, batch, is_train=True):
        if is_train:
            ...
            return outputs, labels, loss_stats, None, None
        else:
            inputs = batch['features'].to(self.device)
            ...
            outputs = self.model(inputs, masks, win_level_features)
            sn_names = batch['id']
            log_times = batch['logtime']
            return outputs, None, None, sn_names, log_times
...
def main(args):
    ...
    ds = time_series_dataset(dataset_path, is_train=args.is_train)
    if args.is_train:
        ...
        val_loader = DataLoader(val_ds, batch_size=args.batch_size//4, num_workers=args.num_workers,
                                shuffle=False, collate_fn=collate_func, sampler=val_sampler)
    else:
        if args.local_rank == 0:
            print("Size: ", len(ds))
            print('# testing circuits: ', len(ds))
        test_sampler = torch.utils.data.distributed.DistributedSampler(ds,
                                                                       num_replicas=args.world_size,
                                                                       rank=args.rank)
        test_loader = DataLoader(ds, batch_size=args.batch_size, num_workers=args.num_workers,
    ...
    # -----------Inferencing--------------
        ...
        with torch.no_grad():
            ...
            args.output_dir = os.path.join(
                args.save_dir, f"{args.local_rank}_result.csv")
