...
class FeatureFactory(object):
    ...
    def process_all_positive_sn(self) -> NoReturn:
        """
        处理所有 sn 文件, 并保存特征, 支持多进程处理以提高效率
        """

        ticket_form = pd.read_csv(self.config.ticket_path)
        sn_files = os.listdir(self.config.data_path)
        sn_names_positive = ticket_form['sn_name'].tolist()
        exist_sn_file_list = os.listdir(self.config.feature_path)

        sn_files = [
            x for x in sn_files 
            if (os.path.splitext(x)[0] in sn_names_positive 
                and x.endswith(self.config.DATA_SUFFIX)
                and x not in exist_sn_file_list)
        ]
        sn_files.sort()
        ...
        if self.config.USE_MULTI_PROCESS:
            ...
            with Pool(worker_num) as pool:
                list(
                    tqdm(
                        pool.imap(self.process_single_positive_sn, sn_files),
                        total=len(sn_files),
                        desc="Generating pos features",
                    )
                )
        else:
            for sn_file in tqdm(sn_files, desc="Generating features"):
                self.process_single_positive_sn(sn_file)

    def process_single_positive_sn(self, sn_file: str) -> NoReturn:
        """
        处理单个 sn 文件, 获取不同尺度的时间窗口特征
        ...
        # 获取处理后的 DataFrame
        new_df = self._get_processed_df(sn_file)
        sn_names_positive = ticket['sn_name'].tolist()

        sn_name, _ = os.path.splitext(sn_file)
        alarm_time = ticket_sn_map[sn_name]

        log_times = new_df["LogTime"].values
        alarm_time_idx = np.searchsorted(log_times, alarm_time, side="left")
        
        # 原来的划分方法
        # new_df["time_index"] = new_df["LogTime"] // self.config.feature_interval
        # 新的划分方法
        new_df["time_index"] = new_df["LogTime"].apply(
            lambda log_time: self.calculate_time_index(log_time, alarm_time, alarm_time_idx)
        )
    ...
    def calculate_time_index(self, log_time, alarm_time, alarm_time_idx):
        """
...
if __name__ == "__main__":
    ...
    feature_factory.process_all_positive_sn()  # 处理所有 SN 文件
    feature_factory.process_all_sn() # 先处理正样本sn，后面就不会重复处理
